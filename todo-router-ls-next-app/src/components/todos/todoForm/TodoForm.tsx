...
const TodoForm:FC<Props> = ({addTodo,placeholder}) => {
  ...
  return (
    ...
    <form className='flex items-center justify-center py-4  mt-4 mb-1' onSubmit={onSubmithandler}>
      ...
      {/* а если у пользователи ширина экрана маленький-например Gaalxy Fold- то применяем другой стиль именно для экрана где макс ширина 280px */}
        <input className='max-[281px]:w-[170px] w-[220px] md:w-[400px]  h-10 outline-none py-1 px-3 rounded-sm font-semibold focus:border-orange-300 focus:border' type="text" value={text} placeholder={placeholder} onChange={(e)=>setText(e.target.value)} />
